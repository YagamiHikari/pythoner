# -*- coding:utf-8 -*-
#模拟登录湖北中医药大学教务管理系统
import urllib,requests
import urllib2,string
import cStringIO,re
from PIL import Image

s=requests.Session()

#获取验证码
login_url=r'http://218.197.176.40/%28trx14ynx5mrpawvrsckaoc45%29/default2.aspx'
s.get('http://218.197.176.40/%28trx14ynx5mrpawvrsckaoc45%29/default2.aspx')
imgRequest=r'http://218.197.176.40/%28trx14ynx5mrpawvrsckaoc45%29/CheckCode.aspx'

userid=raw_input("stuNo:")
pwd=raw_input("Pwd:")
res=s.get(imgRequest)
tempIm=cStringIO.StringIO(res.content)
im=Image.open(tempIm)
im.show()
check=raw_input("check:")

#需要提交的数据
postdata={
    '__VIEWSTATE':'dDwyODE2NTM0OTg7Oz6/Ebx2MPSTY6azYZBe/AC2NAfcrQ==',
    'txtUserName':userid,
    'TextBox2':pwd,
    'txtSecretCode':check,
    'RadioButtonList1':r'学生'
    }
req=s.post(url=login_url,data=postdata)

username=r'张伯成'
posturl='http://218.197.176.40/(4fxwaj55zvzyciqp31ngcrqq)/xscjcx.aspx?xh='+userid+'&xm='+username+'&gnmkdm=N121605'
scoreData={
    'xh':userid,
    'xm':username,
    'gnmkdm':'N121605',
    '__VIEWSTATE':'',
    'btn_zcj':r'历年成绩'
    }
req=s.post(
    url = posturl,
    data = scoreData
    )  
info=req.content
print info
